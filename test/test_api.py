...
class TestApiSearch:
    ...
    @allure.description("Проверка, что при отсутствии phrase возвращается 400 Bad Request.")
    ...
    @allure.description("Поиск по #^ должен вернуть 422 Unprocessable Content.")
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.api
    ...
    @allure.description("Запрос с просроченным токеном должен вернуть 401 Unauthorized.")
    ...
    def test_search_by_author(self):
        ...
        with allure.step("В ответе есть товары"):
            products = data.get("data", {}).get("relationships", {}).get("products", {}).get("data", [])
